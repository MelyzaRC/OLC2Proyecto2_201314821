...
value operation::traducir(environment *env, asttree *tree, generator_code *gen){
    ...
    if(Operator == "+")
    {
        ...
        std::string newMayorTmp = gen->newTemp();
        value op1 = Op_izq->traducir(env, tree, gen);
        value op2 = Op_der->traducir(env, tree, gen);

        if(op1.TipoExpresion == STRING || op2.TipoExpresion == STRING){
            //ERROR SEMANTICO
            std::string contenido_error =  "No se puede comparar un tipo de dato ";
            contenido_error += env->obtenerTipo(op1.TipoExpresion);
            contenido_error += " con un tipo de dato ";
            contenido_error += env->obtenerTipo(op2.TipoExpresion);
            contenido_error += " por medio del operador [>]";
            tree->errores.append(*new error_analisis(Line, Col, 3, contenido_error));
            tree->erroresSemanticos++;
            val= *new value("NULO", false, NULO);
            return val;
        ...
        if(op1.TipoExpresion == BOOL ){
            ...
            if(op2.TipoExpresion == BOOL ){
                ...
                for(int i = 0; i<op1.FalseLvl.size();i++){
                    op2.FalseLvl.append(op1.FalseLvl.value(i));
                }
                ...
                gen->AddLabel(salir);
                //ERROR SEMANTICO
                std::string contenido_error =  "Se detecto un operacion AND con el segundo operador de tipo ";
                contenido_error += env->obtenerTipo(op2.TipoExpresion);
                contenido_error += " - ";
            ...
            gen->AddLabel(salir);
            //ERROR SEMANTICO
            std::string contenido_error =  "Se detecto un operacion AND con el primer operador de tipo ";
            contenido_error += env->obtenerTipo(op1.TipoExpresion);
            contenido_error += " - ";
            contenido_error += " Se esperaba BOOL";
            tree->errores.append(*new error_analisis(Line, Col, 3, contenido_error));
